...
export const CounterWidget = (props) => {
  const { icon, iconColor, category, title, data } = props;
        ...
        <Row className="d-block d-xl-flex align-items-center">
          ...
          <Col xs={12} xl={7} className="px-xl-0">
            <div className="d-none d-sm-block">
              <h5>{category}</h5>
              <p className="mb-1 small text-danger">{title}</p>
            </div>
            {data.map((d,i) => <small key={`stat-${i}`}>{d.country} <FontAwesomeIcon icon={faLongArrowAltRight} size="xs" /> {d.value}<br/></small>)}
          </Col>
        </Row>
      </Card.Body>
    </Card>
